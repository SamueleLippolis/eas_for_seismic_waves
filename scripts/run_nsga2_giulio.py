...
from src.parameter_utils import x_dict_to_vector
from src.optimizers.common import make_vector_objective
from src.optimizers.nsga2_giulio import run_nsga2_giulio
...
EXPERIMENT_PATH = ROOT / "config" / "nsga2_giulio.yaml"
...
def main():
    ...
    objectives_config = {
       "W1": float(weights.get("W1", 1.0)),
        "W2": float(weights.get("W2", 100.0)),
        "epsilon_sigma": float(config["objective"].get("epsilon_sigma", 1.0e-12)),
    }
    ...
    for idx, C_true in enumerate(clay_values):
        ...
        pareto_df = nsga_result["pareto_df"]
        final_population_df = nsga_result["final_population_df"]
        selected = nsga_result["selected_solution"]
        n_eval = nsga_result["n_eval"]

        clay_report_dir = run_dir / f"C_{int(C_true):03d}"
        clay_report_dir.mkdir(parents=True, exist_ok=True)

        pareto_df.to_csv(clay_report_dir / "pareto_front.csv", index=False)
        final_population_df.to_csv(clay_report_dir / "final_population.csv", index=False)
        selected.to_frame().T.to_csv(clay_report_dir / "selected_solution.csv", index=False)

        x_hat = {
            name: float(selected[f"{name}_hat"])
            for name in variable_order
        }

        y_hat = forward_model(x_hat, constants)

        loss_at_recovered_x = float(selected["giulio_scalar_loss"])
        ...
        row = {
            ...
            "loss_at_recovered_x": float(loss_at_recovered_x),
            "selection_rule": "min_giulio_scalar_loss_on_pareto_front",
            "n_eval": None if n_eval is None else int(n_eval),
            "n_pareto_solutions": int(len(pareto_df)),
            "f1_vp": float(selected["f1_vp"]),
            "f2_vs": float(selected["f2_vs"]),
            "f3_inv_sigma": float(selected["f3_inv_sigma"]),
            "e_vp_unscaled": float(selected["e_vp_unscaled"]),
            "e_vs_unscaled": float(selected["e_vs_unscaled"]),
            "e_inv_sigma_unscaled": float(selected["e_inv_sigma_unscaled"]),
        ...
        print(
            ...
            f"loss_hat={loss_at_recovered_x:.6g}, "
